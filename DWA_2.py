...
alpha = 3.0  # heading constant
beta = 1.5   # obstacle distance constant
gamma = 0.5   # velocity constant, better to avoid
...
class DWAController(Node):
    ...
    def dist_from_obstacle_callback(self, msg: LaserScan):
        ...
        self.get_yaw = self.state[2]
        self.scan_x = self.state[0]
        self.scan_y = self.state[1]
    ...
    def dwa_control(self, state, v, w):
        ...
        for vt in np.linspace(dw[0], dw[1], 10):
            for wt in np.linspace(dw[2], dw[3], 10):
    ...
    def calc_trajectory(self, state, v, w):
        traj = []
        for _ in range(int(PREDICT_TIME / DT)): #this is to factor in position at slightly later time as well.
    ...
    def calc_obstacle_cost(self, traj):
        ...
        for x, y, _ in traj:
            for ox_r, oy_r in self.obstacle_pts:
                ...
                ox_w = ox_r * math.cos(self.get_yaw) - oy_r * math.sin(self.get_yaw) + self.scan_x
                oy_w = ox_r * math.sin(self.get_yaw) + oy_r * math.cos(self.get_yaw) + self.scan_y
